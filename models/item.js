...
var ItemSchema = new Schema({
  ...
  order: { type: Number }, //order of item within the parent
...
ItemSchema.statics.byUserAndExternalId = function(user, externalId) {
  return Item
    ...
    .select('_id items parent previous next order visible user type favicon screenshot url externalId browserKey lastSync title deleted'); //We exclude html to speed up
};
...
ItemSchema.statics.by = function(query) {
  return Item
    .findOne(query)
    .select('_id items parent previous next order visible user type favicon screenshot url externalId browserKey lastSync title deleted storageItem')
    .exec();
};
...
ItemSchema.statics.all = function(query) {
  return Item
    .find(query)
    .select('_id items parent previous next order visible user type favicon screenshot url externalId browserKey lastSync title deleted')
    .exec();
}
